...
BinarySearchTree<Key, Value, Comp>::iterator::iterator()
    : mCurrent(NULL)
{
...
{
    //creating a deep copy of the pointer, not the node
...
typename BinarySearchTree<Key, Value, Comp>::iterator& BinarySearchTree<Key, Value, Comp>::iterator::operator++()
{
    ...
    while ((mCurrent->getParent() != NULL) && ((mCurrent->getParent())->getLeft() != mCurrent)) {
